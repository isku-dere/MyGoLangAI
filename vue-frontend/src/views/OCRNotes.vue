  ...
  <div class="ocr-page">
    ...
    <main class="workspace">
      <section class="card queue">
        <div class="title queue-title"><div><b>{{ t.queue }}</b><small>{{ pendingCount }} {{ t.pendingCount }}</small></div><button class="primary" :disabled="!canStartOcr" @click="startAllOcr">{{ t.recognizeAll }}</button></div>
        <div v-for="item in files" :key="item.id" :class="['task', { active: selected && selected.id === item.id, removed: item.removed }]" @click="selectedId = item.id">
          <div class="task-head"><b class="file-name" :title="item.name">{{ item.name }}</b><el-tag class="status-tag" size="small" :type="tagType(item.status)">{{ statusText(item.status) }}</el-tag></div>
          ...
          <div class="actions">
            ...
            <button v-if="item.status === 'ready'" class="primary" @click.stop="startOcr(item)">{{ t.recognizeOne }}</button>
...
<style scoped>
.ocr-page{min-height:100vh;padding:26px;background:linear-gradient(135deg,#17324d,#2f5d62 48%,#f2cc8f);color:#18242f}.hero,.card{background:rgba(255,255,255,.93);border-radius:24px;box-shadow:0 24px 70px rgba(12,27,39,.22);padding:20px}.hero{display:grid;grid-template-columns:auto 1fr 360px;gap:20px;align-items:center;margin-bottom:18px}.hero p{letter-spacing:.22em;color:#d98146;font-weight:800}.hero h1{font-size:42px;margin:6px 0}.upload-panel{display:grid;gap:12px}.drop{display:grid;gap:6px}.workspace{display:grid;grid-template-columns:300px minmax(280px,.85fr) minmax(440px,1.4fr);gap:18px;align-items:start}.bottom{display:grid;grid-template-columns:1.1fr .9fr;gap:18px;margin-top:18px}.title,.task-head,.actions,.doc{display:flex;align-items:center;justify-content:space-between;gap:10px}.title{margin-bottom:14px}.queue-title{align-items:flex-start}.queue-title>div{display:grid;gap:4px}.title small,.muted,.meta{color:#71808b}.task-head{min-width:0}.file-name{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.status-tag{flex:0 0 auto}.task,.doc{display:block;border:1px solid #e4ece7;border-radius:16px;padding:12px;margin-bottom:10px;background:#fbfdf9;cursor:pointer}.doc{display:flex}.doc>div{display:grid;gap:5px;min-width:0}.doc b,.doc span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.task.active{border-color:#d98146;box-shadow:0 12px 30px rgba(217,129,70,.18)}.task.removed{opacity:.45}.actions{justify-content:flex-start;flex-wrap:wrap;margin:10px 0;max-width:100%}.primary{background:#d98146;color:white}.danger{background:#ffe6df;color:#b6402a}button{border:0;border-radius:999px;padding:9px 14px;cursor:pointer;background:#e8efe9;color:#17324d;font-weight:700}button:disabled{opacity:.45;cursor:not-allowed}.preview-stage{height:560px;border:1px solid #e1e8e2;border-radius:18px;background:#fffdf7;display:flex;align-items:center;justify-content:center;overflow:hidden}.preview-stage img{max-width:86%;max-height:86%;transition:transform .2s ease;object-fit:contain;box-shadow:0 12px 32px rgba(23,50,77,.16)}.file-preview{display:grid;gap:10px;place-items:center;color:#71808b;max-width:100%;padding:14px;text-align:center;word-break:break-word}.file-preview b{font-size:52px;color:#d98146}.preview-actions{justify-content:center}.md-box{display:grid;gap:12px;min-height:560px}.md-box.split{grid-template-columns:1fr 1fr}textarea,article.summary-view,.md-box article{width:100%;min-height:560px;border:1px solid #e1e8e2;border-radius:18px;padding:16px;background:#fffdf7;box-sizing:border-box;overflow:auto;line-height:1.7}textarea{resize:vertical;font-family:Consolas,monospace}.summary-editor{min-height:480px}.summary-editor textarea,.summary-editor article{min-height:480px}.summary-view{min-height:320px}.error{color:#c0392b}.empty{text-align:center;color:#71808b;padding:28px;border:1px dashed #cbd7d0;border-radius:18px}.big{min-height:520px;display:grid;place-items:center}.doc span{color:#71808b;font-size:13px}@media(max-width:1280px){.workspace{grid-template-columns:320px 1fr}.editor{grid-column:1/-1}}@media(max-width:900px){.ocr-page{padding:14px}.hero,.workspace,.bottom{grid-template-columns:1fr}.hero{gap:12px;align-items:start}.hero h1{font-size:30px}.hero p{letter-spacing:.12em}.hero,.card{border-radius:16px;padding:14px}.upload-panel{width:100%}.md-box.split{grid-template-columns:1fr}.preview-stage{height:320px}.md-box,textarea,.md-box article{min-height:320px}.summary-editor,.summary-editor textarea,.summary-editor article{min-height:300px}.big{min-height:220px}.actions{gap:8px}.actions .el-input{flex:1 1 100%}.doc{align-items:flex-start}.doc button{flex:0 0 auto}.queue-title,.title{align-items:flex-start;flex-wrap:wrap}:deep(.el-drawer){width:100%!important}}@media(max-width:520px){.ocr-page{padding:10px}.hero h1{font-size:26px}.hero span,.muted,.meta{font-size:13px}.preview-stage{height:260px}.md-box,textarea,.md-box article{min-height:260px}.summary-editor,.summary-editor textarea,.summary-editor article{min-height:260px}button{padding:8px 11px}.doc{display:grid}.doc span{white-space:normal}.file-preview b{font-size:42px}:deep(.el-pagination){white-space:normal;justify-content:flex-start}}
</style>
